...
(async()=>{const browser=await chromium.launch({executablePath:'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe',headless:true});try{
 ...
 async function reachComputer(){if(await page.locator('.room-monitor').isVisible())return;await page.keyboard.down('ArrowRight');await page.waitForTimeout(350);await page.keyboard.press('ArrowUp');await page.waitForTimeout(800);await page.keyboard.up('ArrowRight');await page.locator('.room-monitor').waitFor({state:'visible'});}
 await page.goto('http://localhost:8765/conteudo/3-serie/matematica/tabuada/002/index.html');await page.locator('.room-world').waitFor();
 ...
 if(!(await page.locator('.room-instructions').innerText()).includes('Use o computador'))throw Error('Texto');
 ...
 for(let phase=0;phase<5;phase++){
  ...
  await page.locator('.room-next').click();if(phase<4)await page.locator('.room-monitor').waitFor({state:'hidden',timeout:10000});
 }
 if(await page.locator('.score-value').innerText()!=='610 pontos')throw Error('Pontos');
 await page.locator('.room-replay').click();await page.setViewportSize({width:390,height:844});await reachComputer();
