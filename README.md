...
## Série exibida
Em `config/site.js`, altere `serieAtual: 3` para definir a série padrão antes do login. Depois de entrar, a landing page usa a série salva no perfil do aluno. Para séries cadastradas no catálogo, o livro mostra os cards de matérias: os que tenham jogos recebem `Disponível` e os demais, `Em breve`. Quando a série ainda não estiver cadastrada, o livro mostra um aviso de que novas brincadeiras chegarão em breve.
