(() => {
  ...
  const source = new URL('./app.mjs?v=restore-f1d0745-dblclick', document.currentScript.src).href;
  window.BENA_JOGO = {
    iniciar(container, voltar, partidaInicial) {
      ...
      import(source).then(module => { if (!disposed) cleanup = module.start(container, voltar, partidaInicial); }).catch(error => {
        console.error(error);
        if (!disposed) {
          ...
          container.querySelector('button').onclick = () => window.BenaPartida.recarregar();
        }
      });
